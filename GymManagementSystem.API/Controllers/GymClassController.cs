...
public class GymClassController : BaseController
{
    ...
    [HttpPut("{id:guid}")]
    public async Task<ActionResult<GymClassInfoResponse>> Update(Guid id, GymClassUpdateRequest entity, CancellationToken cancellationToken)
        => HandleResult(await _gymClassService.UpdateAsync(id, entity, cancellationToken));
}
